...
void TravelAgency::readFile(string filePath, int startRow, int flightCount, int hotelCount, int carCount, int travelCount, int customerCount, double totalPrice) {
    ...
    for(int i = startRow; i < data.size(); i++){
        ...
        try{
            if(data.at(i)["type"].is_null() || data.at(i)["type"].empty() || data.at(i)["type"].get<string>().length() == 0 ){
                throw runtime_error("Leeres Attribut 'type' in Zeile " + to_string(i + 1));
            }
            if(data.at(i)["id"].is_null() || data.at(i)["id"].empty() || data.at(i)["id"].get<string>().length() == 0 ){
                throw runtime_error("Leeres Attribut 'id' in Zeile " + to_string(i + 1));
            }
            if(data.at(i)["fromDate"].is_null() || data.at(i)["fromDate"].empty() || data.at(i)["fromDate"].get<string>().length() == 0 ){
                throw runtime_error("Leeres Attribut 'fromDate' in Zeile " + to_string(i + 1));
            }
            if(data.at(i)["toDate"].is_null() || data.at(i)["toDate"].empty() || data.at(i)["toDate"].get<string>().length() == 0 ){
            ...
            if(type == "Flight"){
                if(data.at(i)["fromDest"].is_null() || data.at(i)["fromDest"].empty() || data.at(i)["fromDest"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'fromDest' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["toDest"].is_null() || data.at(i)["toDest"].empty() || data.at(i)["toDest"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'toDest' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["airline"].is_null() || data.at(i)["airline"].empty() || data.at(i)["airline"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'airline' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["bookingClass"].is_null() || data.at(i)["bookingClass"].empty() || data.at(i)["bookingClass"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'bookingClass' in Zeile " + to_string(i + 1));
                }

                if(data.at(i)["fromDest"].get<string>().length() != 3 || data.at(i)["toDest"].get<string>().length() != 3){
                    throw runtime_error("Falsches Flughafenkuerzel in Zeile " + to_string(i + 1));
                }

                //cout << data.at(i)["bookingClass"][0];
                booking = new FlightBooking(id, price, fromDate, toDate, travelId, data.at(i)["fromDest"], data.at(i)["toDest"], data.at(i)["airline"], data.at(i)["bookingClass"].get<string>()[0]);
                flightCount++;
                totalPrice += price;
                ...
                if(data.at(i)["hotel"].is_null() || data.at(i)["hotel"].empty() || data.at(i)["hotel"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'hotel' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["town"].is_null() || data.at(i)["town"].empty() || data.at(i)["town"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'town' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["roomType"].is_null() || data.at(i)["roomType"].empty() || data.at(i)["roomType"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'roomType' in Zeile " + to_string(i + 1));
                }
                booking = new HotelBooking(id, price, fromDate, toDate, travelId, data.at(i)["hotel"], data.at(i)["town"], data.at(i)["roomType"]);
                hotelCount++;
                totalPrice += price;
            }

            else if(type == "RentalCar"){
                if(data.at(i)["pickupLocation"].is_null() || data.at(i)["pickupLocation"].empty() || data.at(i)["pickupLocation"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'pickupLocation' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["returnLocation"].is_null() || data.at(i)["returnLocation"].empty() || data.at(i)["returnLocation"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'returnLocation' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["company"].is_null() || data.at(i)["company"].empty() || data.at(i)["company"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'company' in Zeile " + to_string(i + 1));
                }
                if(data.at(i)["vehicleClass"].is_null() || data.at(i)["vehicleClass"].empty() || data.at(i)["vehicleClass"].get<string>().length() == 0 ){
                    throw runtime_error("Leeres Attribut 'vehicleClass' in Zeile " + to_string(i + 1));
                }
                booking = new RentalCarReservation(id, price, fromDate, toDate, travelId, data.at(i)["pickupLocation"], data.at(i)["returnLocation"], data.at(i)["company"], data.at(i)["vehicleClass"]);
            ...
            msgBox.setInformativeText("Wenn Sie die Datei bereits korrigiert haben, wählen Sie 'Wiederholen'. Wählen Sie 'Verwerfen', um alle Buchungen zu löschen "
            ...
            cout << "1234";
            msgBox.setWindowTitle("Fehler beim Einlesen der Buchungen" );
            msgBox.setText("Attribut price hat keinen numerischen Wert in Zeile " + QString::fromStdString(to_string(i + 1)));
            msgBox.setInformativeText("Wenn Sie die Datei bereits korrigiert haben, wählen Sie 'Wiederholen'. Wählen Sie 'Verwerfen', um alle Buchungen zu löschen "
                                      "und 'Abbrechen', um die vorhandenen Buchungen stehenzulassen und diesen Dialog zu verlassen");
            ...
            QAbstractButton *doNotCloseButton = msgBox.addButton("This button will not close anything", QMessageBox::ActionRole);
    ...
    msgBox.setText(QString::fromStdString("Es wurden " + to_string(flightCount) + " Flugbuchungen, " + to_string(hotelCount) + " Hotelbuchungen und " + 
    to_string(carCount) + " Mietwagenreservierungen im Gesamtwert von " + to_string(totalPrice) + " Euro eingelesen. Es wurden " + to_string(travelCount) + " Reisen und "
    + to_string(customerCount) + " Kunden angelegt. Der Kunde mit der ID 1 hat " + to_string(getTravelCount(1)) + " Reisen gebucht. Zur Reise mit der ID "
